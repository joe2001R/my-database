...
{
    ENSURE( *node_get_type(old_leaf_node) == LEAF_NODE, "Error: split and insert called on non leaf node");
...
{
    ENSURE(leaf_node_find_row(node, key, NULL) == NULL, "Error: insert already existing key %d", key);
