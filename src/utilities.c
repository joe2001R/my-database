...
void string_buffer_append(string_buffer *buffer, const char *string)
{
    ...
    ensure(buffer->string!=NULL,"Error: not enough memory to realloc call in `string_buffer_append`\n");
...
void string_buffer_append2(string_buffer *buffer, const char *format, ...)
{
    ...
    ensure(size>0,"Error: vsnprintf was unsuccessful in `string_buffer_append2`");
